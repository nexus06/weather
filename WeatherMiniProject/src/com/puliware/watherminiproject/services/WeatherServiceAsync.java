 ...
 * 
 * @brief This class uses asynchronous AIDL interactions to expand acronyms via
 ...
 *        making one-way method calls on the AcronymRequest object asking this
 *        Service to lookup the Acronym's meaning, passing in an AcronymResults
 *        object and the Acronym string. After the lookup is finished, this
 *        Service sends the Acronym results back to the Activity by calling
 *        sendResults() on the AcronymResults object.
 * 
...
public class WeatherServiceAsync extends LifecycleLoggingService {
	/**
	 * Factory method that makes an Intent used to start the AcronymServiceAsync
	 ...
	 * proper Intent. Returns the implementation of AcronymRequest, which is
	...
	/**
	 * The concrete implementation of the AIDL Interface AcronymRequest, which
	 * extends the Stub class that implements AcronymRequest, thereby allowing
	 ...
	 */
	private final WeatherRequest.Stub mAcronymRequestImpl = new WeatherRequest.Stub() {
		/**
		 ...
		 * Acronym Web service and then sends the results back to the Activity
		 * via a callback.
		 */
		@Override
